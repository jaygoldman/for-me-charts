...
- **`state`** is the single source of truth: `{ title, emoji, maxAge, axis{x,y}, axisEmoji{yHigh,yLow,xLow,xHigh}, fonts{title,label}, colors{bg,grid,line,line2,text}, dottedGrid, numericLabels, gradient, xSteps, ySteps, points:[{y,corner}] }`. `y` is `0..1` (0 = bottom). There is one handle (`point`) on **every vertical age line**, so `points.length === xSteps + 1`; changing `xSteps` resamples `points` (enforced in `expand()` too). `ySteps` sets the horizontal divisions. `axis.x`/`axis.y` are single centered labels; `axisEmoji` are drawn at the axis endpoints. `numericLabels` toggles the age/percent tick numbers; `dottedGrid` dashes the grid; `gradient` strokes the line with a `line`→`line2` `linearGradient` (defined inline so it survives export). `fonts.*` are keys into the `FONTS` map.
